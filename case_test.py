"""Does LOWERCASING the spans make the dictionary more coherent?

Casing is a plausible nuisance direction: "Valley girl" and "valley girl" induce different
activations, so case variance inflates cell-to-cell disagreement and lowers the consistency
statistic without carrying concept information. The mining dedup already case-folds the HASH, so
variants collapse to one entry -- but the surviving span keeps whatever casing it happened to have,
which means the bank has arbitrary casing rather than none.

Measured on the SAME spans, original vs lowercased, in raw and J-space:
  * rho mean and spread -- does lowercasing raise consistency, and does it keep the dynamic range
    a filter needs (the range is the thing currently too narrow)
  * concept separation against 20 FineFineWeb domain labels (d-prime, AUC)
Run BEFORE the 10M pass, because getting this wrong costs 1.2 GPU-days of redoing it.
...
app = modal.App("celeste-case-test")
...
VARIANTS = {"original": SPANS, "lowercase": [t.lower() for t in SPANS]}
XV = {}
for vname, texts in VARIANTS.items():
    acc = []
    for a in range(0, len(texts), 300):
        acc.append(read_cells(texts[a:a+300]))
        print("[read] %s %d/%d" % (vname, min(a+300, len(texts)), len(texts)), flush=True)
    XV[vname] = torch.from_numpy(np.concatenate(acc, 0)).to(dev)
n_changed = sum(1 for a, b in zip(VARIANTS["original"], VARIANTS["lowercase"]) if a != b)
print("[case] %d of %d spans actually change when lowercased (%.0f%%)"
      % (n_changed, len(SPANS), 100*n_changed/len(SPANS)), flush=True)
X = XV["original"]
...
res = {}
for vname in ("original", "lowercase"):
  for space in ("raw", "jspace"):
    X = XV[vname]
    key = "%s/%s" % (vname, space)
    Z = project(X, space)
    ...
    res[key] = {"rho_mean": float(rho.mean()), "rho_sd": float(rho.std()),
    ...
    print("   concept separation: within %.4f  across %.4f  gap %+.4f  d' %.3f  AUC %.4f"
          % (res[key]["within"], res[key]["across"], res[key]["gap"], dp, res[key]["auc"]), flush=True)
    print("   rho (filter axis) : mean %.4f  sd %.4f  p5-p95 %.4f-%.4f  (range %.4f)"
          % (res[key]["rho_mean"], res[key]["rho_sd"], res[key]["rho_p5"],
             res[key]["rho_p95"], res[key]["rho_p95"]-res[key]["rho_p5"]), flush=True)

print("\n%-22s %8s %8s %8s | %9s %9s %9s" % ("variant/space","gap","d'","AUC","rho mean","rho sd","rho range"))
for k, v in sorted(res.items(), key=lambda kv: -kv[1]["auc"]):
    print("%-22s %8.4f %8.3f %8.4f | %9.4f %9.4f %9.4f"
          % (k, v["gap"], v["dprime"], v["auc"], v["rho_mean"], v["rho_sd"], v["rho_p95"]-v["rho_p5"]))
json.dump({"n_spans": len(SPANS), "domains": doms, "spaces": res},
          open("/vol/results_case_test.json","w"), indent=1)
