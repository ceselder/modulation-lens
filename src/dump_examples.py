...
N_LOW, N_MIX, SAMP, TOPK, MAXNEW = 10, 5, 2, 12, 32
dev, EOS = "cuda", 248046
...
def readouts(v, tag, extra=""):
    ...
    jl = " ".join(tok.decode([i]).strip() for i in torch.topk(lg[0], TOPK).indices.tolist())
    outs = []
    for _ in range(SAMP):
    ...
    print("  J-LENS : %s" % jl[:150], flush=True)
    for k, o in enumerate(outs):
        print("  MAEMM%d : %s" % (k + 1, o[:150]), flush=True)


print("=" * 100)
print("LEAST-RELIABLE ATOMS  (reliability is the split-half template agreement; dataset floor 0.65)")
print("=" * 100)
for i in order[:N_LOW]:
    readouts(V[i], "LABEL  : %r" % labs[i][:70], "   [reliability %.3f]" % R[i])
